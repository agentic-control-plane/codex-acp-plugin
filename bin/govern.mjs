...
//   "[ACP] Denied by policy: ..."                    (policy said no)
//   "[ACP] Gateway unreachable (...) — ... blocked"  (unattended tier, fail-closed)
...
//
// Unreachability posture (#385, decided 2026-07-21; ported from
// claude-code-acp-plugin in 0.6.5 — before that, this file's header claimed
// fail-open while the code failed closed on every tier):
//   interactive  → fail OPEN, loudly — "[ACP] ⚠ UNGOVERNED" + ~/.acp/lapse.log
//   subagent/background/api → stay fail-CLOSED (nobody is watching)
// Policy denies are unaffected — this only governs what happens when the
// policy cannot be ASKED at all.
...
async function handlePreToolUse() {
  ...
  // HTTP 200 — an ALLOW — but the single 4s attempt had already aborted, and
  // at an unattended tier the fail-closed posture turned that allow into a
  // deny. Slow answers are cold starts, so the retry lands on a warm
  // instance. The split keeps the total under the harness hook timeout —
  // overrunning it would be killed mid-flight and denied with no reason.
  ...
  try {
    try {
      ...
      // re-rolled. A second failure falls through to the fail posture.
